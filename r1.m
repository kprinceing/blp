...
%search below.
    ...
    % In the lines below, s_jt : brand share in each market , dim : 2256 *1
    % temp : cumulative sum of the s_jt
    % sum1 : takes the total share for each market
    % sum 2: first difference to get total share for each market
    % outshr : outside good share for each market. 
    % sum 1, sum2 : 94*1 
    % temp, outhsr : 2256*1
...
disp(['Object outshr dimensions: ' num2str(size(outshr)) ])
    ...
    % It's used as a initial value for the calculation. 
...
clear mid y outshr t oldt2 mvalold temp sum1
    ...
    % vfull and dfull are the unobserved and observed individual
    % characteristics matrix. 
    % size of vfull : 2256*80
    % size of dfull : 2256*80
vfull = v(cdid,:);
dfull = demogr(cdid,:);
